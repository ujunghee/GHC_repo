...
기존 챗봇 저장소에 이 UI를 적용하는 경우, 이 feature를 별도 앱으로 새로 띄우거나 iframe으로 얹지 않습니다. 기존 챗봇의 로그인, 사용자 세션, 대화방, 대화 기록, 질문 전송, AI 응답 스트리밍, 중단/재생성/복사, 파일 업로드, 오류/로딩 처리, 백엔드 API, DB 연결은 유지하고, 기존 챗봇 UI만 이 feature의 화면 구조와 CSS로 교체합니다.
...
`data.ts`는 화면과 타입을 설명하기 위한 mock입니다. 운영 통합에서는 기존 챗봇 API 응답을 GHC UI 타입으로 변환하는 adapter를 만들어 연결합니다.
...
- 채팅 입력창에 첨부한 이미지는 고정 preview로만 표시하고, 이동/회전/resize 편집 UI를 제공하지 않습니다.
- 이미지 이동/회전/resize/투명도 조정은 지도 위 도면 overlay에서만 제공합니다.
...
4. `지도에서 보기`를 누른 후보만 지도 위 이미지 배치 UX로 진입합니다.
5. 최초 진입 시 이미지는 바로 적용 완료 상태로 지도 좌표에 고정됩니다.
6. 우하단 관리 패널에서 `이미지 수정` 또는 `이미지 삭제`를 선택할 수 있습니다.
7. `이미지 수정`을 누른 경우에만 지도 위 편집 overlay와 이미지 조정 패널이 열립니다.
8. 편집 후 `이미지 적용 완료`를 누르면 기존 복사 토스트와 같은 UI로 `적용 완료`가 표시됩니다.
9. 최소화된 지도 설정 팝업은 헤더를 드래그해 위치 이동할 수 있습니다.
