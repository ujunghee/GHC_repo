...
이 문서는 발굴보고서 메인 검색 화면을 현재 프로토타입과 최대한 동일하게 재구현하기 위한 최종 인수인계 문서이다.
AI 또는 개발자가 새로 구현할 때는 이 문서를 1순위 기준으로 사용한다.
...
메인 검색 화면은 사용자가 발굴보고서를 검색하고, 검색 도우미에서 단서를 추가하고, 보고서를 최대 2건까지 선택한 뒤 챗봇 화면으로 진입하는 첫 화면이다.
...
그룹이 3건 이상이면 즉시 이동하지 않고:

1. 앞 2건만 선택 상태로 만든다.
2. 토스트를 띄운다.
3. 사용자가 하단 CTA를 누르게 한다.
...
- 최대 2건만 선택 가능
- 3번째 선택 시 선택 상태는 변하지 않는다.
- `최대 2건만 가능합니다` 토스트를 1800ms 표시한다.
...
```txt
solvek-react/src/features/report-search/MAIN_SEARCH_FINAL_HANDOFF_UPDATED.md를 최우선 기준으로 발굴보고서 메인 검색 화면을 구현해줘.
