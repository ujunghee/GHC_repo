...
- 카드를 클릭하면 선택/해제만 처리합니다.
- 선택 가능한 최대 수와 채팅 가능 여부는 채팅 세션 생성 API 응답을 기준으로 판단합니다.

### 12. 그룹 채팅하기

- 그룹 보고서 id 전체로 `onStartChat(reportIds)`를 호출합니다.
- 프론트에서 앞 2건 또는 5건으로 자르지 않습니다.
- API가 채팅 시작을 거절하면 응답 메시지를 토스트로 표시합니다.

### 13. 하단 채팅 CTA

- `selectedReports.length > 0`이면 하단 fixed CTA를 표시합니다.
- CTA 클릭 시 선택 보고서 id 전체로 `onStartChat(reportIds)`를 호출합니다.
...
| `toastMessage` | `string \| null` | API 실패 또는 네트워크 오류 토스트 문구 |
...
- 하단 `채팅하기` CTA를 숨깁니다.
- 진행 중인 채팅 시작 오류 토스트가 있으면 닫을 수 있습니다.
...
- 선택 개수는 프론트에서 임의로 제한하지 않습니다.
- 선택 개수 초과 토스트는 카드 클릭 시점이 아니라 채팅 세션 생성 API 실패 시점에 표시합니다.
...
- 그룹 보고서 id 전체로 채팅 세션 생성을 시도합니다.
- API 성공 시 챗봇으로 이동합니다.
- API 실패 시 응답 메시지를 토스트로 표시합니다.
...
- `MainSearchPage`가 선택 보고서 id 전체로 `onStartChat(reportIds)`를 호출합니다.
...
보고서 선택 관련 토스트는 프론트 임의 제한이 아니라 채팅 세션 생성 API 응답을 기준으로 표시합니다.
...
- API가 선택 개수 초과를 반환한 경우: API `message` 표시
- API가 서버 처리량 부족, 보고서 준비 미완료, 권한 오류를 반환한 경우: API `message` 표시
- API 메시지가 없는 실패: fallback 문구 표시
- 네트워크 오류 또는 timeout: 네트워크 fallback 문구 표시
...
- 보고서 선택을 프론트에서 임의 제한하지 않는가
- 검색 도우미 옵션은 마우스와 키보드로 선택 가능한가
- 선택한 단서 칩 초기화가 가능한가
- 활성 단서 칩에 체크 아이콘이 보이지 않는가
- 활성 키워드별 그룹 결과가 표시되는가
- 그룹 헤더의 `채팅하기`가 그룹 보고서 id 전체로 채팅 시작을 시도하는가
- 채팅 세션 생성 API 실패 시 응답 메시지 토스트를 표시하는가
