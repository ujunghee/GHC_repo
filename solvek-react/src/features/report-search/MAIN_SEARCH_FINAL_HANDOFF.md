...
이 문서는 발굴보고서 검색 메인 화면을 다른 개발자 또는 개발자용 AI가 재구현할 때 기준으로 삼는 최종 전달 문서입니다. 현재 React 프로토타입의 기능, 컴포넌트, 상태, 이벤트, 데이터, 애니메이션, 접근성, API 연결 기준을 모두 포함합니다.
...
메인 화면은 사용자가 발굴보고서를 검색하고, 검색 도우미 단서를 추가하고, 대화할 보고서를 최대 2건까지 선택한 뒤 챗봇 화면으로 진입하는 첫 화면입니다.
...
- `toastVisible`: `최대 2건만 가능합니다` 토스트 표시 여부입니다.
...
- 항상 앞 2건까지만 넘깁니다.
- 실제 선택 제한도 2건이므로 일반적으로 `selectedReports` 전체가 전달됩니다.
...
- 그룹 보고서가 1~2건이면 즉시 채팅 화면으로 이동합니다.
- 그룹 보고서가 3건 이상이면 앞 2건만 선택 상태로 만들고 토스트를 띄웁니다.
- 3건 이상일 때는 즉시 이동하지 않고 사용자가 하단 CTA를 누르게 합니다.
...
아래 문장을 개발자용 AI에게 전달하면 됩니다.

```txt
solvek-react/src/features/report-search/MAIN_SEARCH_FINAL_HANDOFF.md를 최우선 기준으로 발굴보고서 메인 검색 화면을 구현해줘.
