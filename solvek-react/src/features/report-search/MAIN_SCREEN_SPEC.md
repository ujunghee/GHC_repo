...
- 보고서를 선택합니다. 선택 개수는 프론트에서 임의로 제한하지 않습니다.
...
- `selectedReports`: 사용자가 선택한 보고서 id입니다. 프론트에서 개수를 임의 제한하지 않습니다.
- `toastMessage`: 채팅 세션 생성 API 실패 또는 네트워크 오류 시 표시할 토스트 문구입니다.
...
- 초기화 버튼은 칩 전체를 지웁니다.
- 칩 영역 자체에는 height/opacity transition을 둘 수 있습니다.
- 검색 결과 영역 위치 이동에는 `LayoutGroup`, `layout`, `layout="position"` 애니메이션을 사용하지 않습니다.
- 스크롤을 내린 상태에서 검색어를 입력해도 결과 콘텐츠가 검색바와 겹치며 내려오는 애니메이션이 없어야 합니다.
...
- 선택되지 않은 보고서는 선택합니다.
- 이미 선택된 보고서는 선택 해제합니다.
- 선택 개수 초과 여부는 카드 클릭 시 판단하지 않습니다.

토스트 표시 기준:

- 채팅 세션 생성 API가 `REPORT_LIMIT_EXCEEDED`를 반환하면 API 메시지를 표시합니다.
- API가 `SERVER_BUSY`, `REPORT_NOT_READY`, `FORBIDDEN` 등 채팅 시작 불가 사유를 반환하면 API 메시지를 표시합니다.
- API 메시지가 없으면 fallback 문구를 표시합니다.
- 네트워크 오류 또는 timeout이면 네트워크 fallback 문구를 표시합니다.
...
- 접근성 속성은 유지합니다.
- 검색 input 변경, 빈 상태 전환, 일반/그룹 결과 전환처럼 결과 영역 위치가 크게 바뀌는 이벤트에는 위치 보간 애니메이션을 걸지 않습니다.
...
프론트 임의 선택 제한 없음, API 응답 기반 토스트, 하단 채팅 CTA 규칙을 반드시 유지해줘.
