...
- 선택 가능한 보고서 수와 채팅 가능 여부는 채팅 세션 생성 API 응답을 기준으로 판단합니다.
- 결과 없음 상태에서는 선택 보고서를 초기화합니다.
...
- 칩 영역 자체의 등장/퇴장에는 height/opacity transition을 사용할 수 있습니다.
- 검색어 입력, 결과 전환, 단서 초기화로 아래 결과 영역 위치가 바뀔 때는 layout position 애니메이션을 사용하지 않습니다.
...
- 그룹에 포함된 보고서 id 전체를 상위로 전달합니다. 개수 제한과 실패 메시지는 채팅 세션 생성 API 응답을 기준으로 처리합니다.
...
- 보고서 선택 개수는 프론트에서 제한하지 않습니다.
- 하단 `채팅하기`는 선택된 보고서 id 전체를 채팅 시작 흐름으로 전달합니다.
- 그룹 `채팅하기`는 그룹 보고서 id 전체를 채팅 시작 흐름으로 전달합니다.
- 선택 개수 초과, 서버 처리량 부족 등 채팅 시작 불가 토스트는 API 응답 메시지를 기준으로 표시합니다.
...
- `selectedReports`는 사용자가 선택한 보고서 id 전체를 보관합니다. 선택 개수 제한은 API 응답을 기준으로 처리합니다.
...
- 메인 검색 화면의 결과 영역에는 `LayoutGroup`, `layout`, `layout="position"`을 사용하지 않습니다.
- 검색 input 변경, 빈 상태 전환, 일반 결과/그룹 결과 전환은 즉시 재배치합니다.
- 스크롤을 내린 상태에서 검색해도 결과 콘텐츠가 검색바와 겹치며 미끄러져 내려오면 안 됩니다.
- 단서 칩 wrapper는 height/opacity/y 출입 애니메이션만 사용하고 위치 보간은 하지 않습니다.
...
- 그룹 헤더 `채팅하기`는 그룹의 report `id` 목록 전체를 채팅 시작 입력값으로 사용합니다.
- 채팅 세션 생성 API가 개수 초과나 서버 처리 불가를 반환하면 응답의 사용자 표시용 `message`를 토스트로 표시합니다.
- API 메시지가 없거나 네트워크 오류가 발생하면 프론트 공통 fallback 토스트를 표시합니다.

API가 붙으면 화면 컴포넌트는 가능하면 변경하지 않고, 데이터 공급 위치만 교체합니다.

## Toast Rules

보고서 선택 관련 토스트는 프론트 임의 판단이 아니라 API 결과에 맞춥니다.

- 카드 선택/해제 성공: 토스트 없음
- 하단 `채팅하기` API 성공: 토스트 없음, 채팅 화면 이동
- 그룹 `채팅하기` API 성공: 토스트 없음, 채팅 화면 이동
- API `REPORT_LIMIT_EXCEEDED`: API 메시지 표시
- API `SERVER_BUSY`, `REPORT_NOT_READY`, `FORBIDDEN`: API 메시지 표시
- API 메시지 없음: `채팅을 시작할 수 없습니다. 잠시 후 다시 시도해주세요.` 표시
- 네트워크 오류: `서버 연결이 원활하지 않습니다. 잠시 후 다시 시도해주세요.` 표시
- 결과 없음 상태: 선택 상태만 초기화, 토스트 없음
